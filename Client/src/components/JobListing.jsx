            ...
            {filteredJobs.length >0 && (
                <div className='flex items-center justify-center space-x-2 mt-10'>
                    ...
                    {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_,index)=>(
                        <button onClick={() => setCurrentPage(index+1)} key={index} className={`w-10 h-10 items-center justify-center border border-gray-300 rounded ${currentPage === index +1 ? 'bg-blue-100 text-blue-500' : 'text-gray-500'}`}>{index+1}</button>
                    ))}
